...
public class HttpProxyCacheServer {
  ...
  private final List<OnErrorListener> listeners = new CopyOnWriteArrayList<>();
  private final UiErrorListener errorListener;
  ...
  private HttpProxyCacheServer(Config config) {
    ...
    try {
      ...
      this.errorListener = new UiErrorListener(listeners);
   ...
   * 2nd parameter set to {@code true}.
   *
  ...
  public void shutdownClients() {
    synchronized (clientsLock) {
      for (HttpProxyCacheServerClients clients : clientsMap.values()) {
        touchFileSafely(getCacheFile(clients.getUrl()));
        clients.shutdown();
      }
      clientsMap.clear();
    }
  ...
  private void closeSocketOutput(Socket socket) {
    try {
      ...
      LOG.warn(
          "Failed to close socket on proxy side: {}. It seems client have already closed " +
          "connection.",
          e.getMessage());
    }
  }
  ...
  public boolean removeCache(String path) {
    return getCacheFile(path).delete();
  }

  public void registerOnErrorListener(OnErrorListener listener) {
    if (!listeners.contains(listener)) {
      this.listeners.add(listener);
    }
  }

  public void unregisterOnErrorListener(OnErrorListener listener) {
    if (listeners.contains(listener)) {
  ...
  private static final class UiErrorListener extends Handler implements OnErrorListener {

    private final List<OnErrorListener> listeners;

    private UiErrorListener(List<OnErrorListener> listeners) {
