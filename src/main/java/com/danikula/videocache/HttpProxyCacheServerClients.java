...
final class HttpProxyCacheServerClients {
  ...
  private volatile HttpProxyCachePartial proxyCache;
  ...
  public void processRequest(GetRequest request, Socket socket)
    ...
    try {
      ...
      proxyCache.processRequest(request, socket, config.timeout, config.sslContext);
    } finally {
  ...
  private HttpProxyCachePartial newHttpProxyCache() throws ProxyCacheException {
    HttpUrlSource source = new HttpUrlSource(
        url,
        config.sourceInfoStorage,
        config.headerInjector,
        config.timeout,
        config.sslContext);
    FileCache cache = new FileCache(config.generateCacheFile(url),
                                    config.fileRange);
    HttpProxyCachePartial httpProxyCache = new HttpProxyCachePartial(source, cache);
